...
- Putting thread selection in a worker means a browser without `Worker` has to be handled: an export under 5 MB is selected on the main thread instead, and a larger one drops the message section rather than freezing the page. A worker that reports a definite failure is not retried on the main thread, because it has already parsed that file with that code. A failure it reports under id zero, meaning it never read the request or fell over outside it, is retried, because nothing about the file has been established.
...
- Thread selection deliberately diverges from `MessagesAnalytics` in three ways, because the export answers a different question. It parses `CONTENT` verbatim rather than through the spreadsheet cleaner, which quote-prefixes formulas and strips control characters. It resolves the account owner itself, consulting the connections file first (you are never in your own connections) before falling back on conversation coverage, and where that still leaves no unique winner it reports `direction: "unknown"` rather than labelling a message with a name it is not sure of. Reading the connections file during a messages export is the one extra local file the export touches, and nothing about it leaves the browser. And it keeps `LinkedIn Member` conversations, scoping each anonymous correspondent to its own `CONVERSATION ID`: analytics is right to drop the placeholder so unrelated strangers do not merge into one contact, but dropping it here would silently delete a recent conversation from the document.
