...
| GitHub Apps (primary, escalation, and audit, see Token model) | Same GitHub account (App owner)                                      | Rotate by generating a new private key in each App's settings, then update the matching Client ID repository variable and private-key secret. If an App is lost, the maintenance workflows degrade gracefully (they skip when credentials are absent). |
...
| PyPI trusted publisher                                        | Maintainer's PyPI account (OIDC trusted publishing, no stored token) | PyPI account recovery; re-configure the trusted publisher under the project publishing settings, with the environment name set to `pypi` so the reviewer gate is enforced. No API token exists to rotate.                                              |
...
| Actions           | Actions are selected, all current references require full commit SHAs, GitHub-owned Actions are allowed, verified Marketplace Actions are not implicitly allowed, and the explicit third-party allowlist matches the workflows; GitHub App Client ID variables `APP_ID`, `ESCALATION_APP_ID`, and `AUDIT_APP_ID`, and secrets `APP_PRIVATE_KEY`, `ESCALATION_APP_PRIVATE_KEY`, and `AUDIT_APP_PRIVATE_KEY`, all exist |
| Environments      | The `pypi` environment requires `Hermione-Granger-1176` as a reviewer, keeps administrator bypass enabled for the sole maintainer, does not prevent self-review, and admits deployments only from tags matching `v*`                                                                                                                                                                                                  |
| Release tags      | The active `Protect version tags` ruleset targets `refs/tags/v*` and blocks creation, deletion, and non-fast-forward updates                                                                                                                                                                                                                                                                                          |
...
**This target needs a GitHub App token, never `GITHUB_TOKEN`.** Reading branch protection, Actions policy, environment protection, and rulesets needs `administration: read`, and listing secrets needs `secrets: read`; `GITHUB_TOKEN` can grant neither. Neither writeback App carries them either, deliberately, so [`audit-repo-settings.yml`](#scheduled-audit-run) runs it weekly on the read-only audit App instead.
...
The release tag ruleset is audited because it controls the identity and mutability of published versions. Its bypass actors are deliberately not audited: GitHub can omit that field from a ruleset response for a read-only token, so checking it would turn a healthy scheduled audit into a permission failure. Other unrelated rulesets are ignored.

The PyPI trusted publisher is unwatched for a different reason: it is not a GitHub setting at all. It lives in the PyPI project's publishing settings, and PyPI exposes no read API for it, so no audit run from here can see it. Its expected configuration is owner `Hermione-Granger-1176`, repository `linkedin-analyzer`, workflow `publish.yml`, and environment `pypi`. That last field is the load-bearing one. Left blank, PyPI accepts a token from any job in the repository, and the `pypi` environment's reviewer rule stops being a publishing gate at all, because a job that never enters the environment can still mint publishing credentials. PyPI emails the project owner when an unconstrained publisher is used from a job that does declare an environment. Treat that mail as drift on this setting and re-check it by hand.
...
Three GitHub Apps provide elevated permissions beyond the default `GITHUB_TOKEN`, each scoped to a single role. The three ID variables contain the corresponding GitHub App Client IDs, and their repository variable names remain unchanged. The split mirrors the token model documented in the `artifacts` repository.

| App                    | Client ID variable  | Private-key secret           | Used for                                        |
| ---------------------- | ------------------- | ---------------------------- | ----------------------------------------------- |
...
- `metadata: read` (implicit, required to call any repository endpoint)
- `administration: read` (branch protection, Actions policy, environments and their deployment branch policies, and rulesets)
...
- Configure PyPI trusted publishing for this repository (OIDC) so `publish.yml` can publish without `PYPI_API_TOKEN`. Set the publisher's environment name to `pypi` rather than leaving it blank: an unconstrained publisher accepts a token from any job in the repository, which silently voids the `pypi` environment's reviewer gate. See [Settings this audit deliberately does not check](#settings-this-audit-deliberately-does-not-check) for the full expected configuration.
