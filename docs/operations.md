...
- Docker image publish includes a Trivy scan gating every severity. `ignore-unfixed` is set, so a publish fails only on findings that already have a fix available.
