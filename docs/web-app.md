...
- Use the floating help button in the bottom-right corner to replay the tutorial for the current page. It shows Pip peeking out of the dashed circle with a small `?` floating beside his head.
...
- Pip guides the tour from a top corner of the callout card. He takes the corner away from the target, so he never sits on the pointer arrow or the highlighted element, and mirrors to gesture across the card toward it. He presents through the tour and waves on the final step. He is decoration: aria-hidden, no pointer events, and positioned out of flow, so he adds nothing to the card's own size. The title gives up a gutter beside him, and that gutter moves from one side to the other when he flips, so the card measures the same whichever way he faces.

## Mascot

Pip is the hand-drawn character who turns up across the app: waving beside the hero title, scribbling in the loading overlays, waiting over the top of an empty box, panicking next to a clean error, catching a file dropped on the upload zone, cheering a finished export, stapling a built PDF, rubbing the Files card clean after a wipe, leaning on the Insights and Messages tips, peeking out of the floating help button, guiding the tutorial, and reacting in the corner of every insight card. He is decoration everywhere: `aria-hidden`, `focusable="false"`, and no pointer events.
...
Two behaviors are driven from JavaScript rather than by CSS alone. `shared/ui/alive.js` gives the hero Pip on **Home** a life of his own: his eyes follow the pointer, he yawns after twenty quiet seconds, and he flinches when the theme is switched. All of it sits out under `prefers-reduced-motion: reduce`, and the gaze also sits out on a device without a fine pointer, where there is nothing to follow. `features/upload/mascot.js` runs the drop-zone catcher: he comes up behind the zone's edge with his arms out while a file is over it, hugs the page once it lands, and is put away on a timer rather than on `animationend`, because a pose whose screen is hidden underneath it would otherwise stay on screen for the rest of the session. `shared/ui/mascot.js` owns the rest of the event-driven moments: the ink splat under a clicked button, the cheer, the staple, and the eraser sweep.

Four places hold the drawings, and each owns one thing:

| Where                                                          | Owns                                                                                                                                                                             |
| -------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `<defs>` in `web/index.html`                                   | The shared `#pipWobble` ink filter, one `<g id="pip-...">` per pose used more than once, and the parts more than one pose is built from: the two hands and the standing body     |
| `web/src/styles/components/mascot.css`                         | The character: ink and fill classes, idle loops, one-shot moves, the reduced-motion guard, and the placement of every pose whose surface has no stylesheet of its own            |
| The stylesheet that owns a surface                             | Where a pose sits on that surface: size, offset, mirroring (`overlays.css` for the loader, `insights.css` for the card stamps, `tutorial.css` for the guide and the help button) |
| `reactions.js`, `tutorial/mascot.js` and `shared/ui/mascot.js` | The poses built in JavaScript, drawn with the same `pip-*` classes and composed from `shared/ui/pip-parts.js`                                                                    |

A pose drawn in more than one place is defined once in the `<defs>` block at the top of `index.html`, and each placement is a wrapper `<svg>` carrying its own class and `viewBox` around a `<use href="#pip-...">`. That covers `#pip-tipster` (the Insights tip and the Messages tip) and `#pip-loader` (the content loading overlay and the upload progress overlay). Parts shared by more than one pose are defined the same way and placed by a bare `<use>` inside the drawing that wants them, under whatever transform puts them where it needs them: `#pip-hand-open` for the hero's wave and the catcher's two raised hands, `#pip-hand-closed` for the staple press, the eraser scrub and both dust hands, and `#pip-body-standing`, hood to shoes with no arms, for the staple and the eraser. The loader is drawn around that same standing body but keeps its own copy: its hair sits inside the animated `.pip-flick` group and its mouth is a different line, so its body is not the same drawing. Poses drawn only once stay inline where they appear, which is where all five empty-state poses live.

Two rules follow from how `<use>` works, and both are load-bearing:

- A `<use>` builds a shadow tree, and a selector written outside the placement cannot reach into it. Anything a pose needs must therefore be a class on the definition itself. The poses styled from an ancestor (`.is-cheering .pip-jump`, `.is-stapling .pip-press`, `.is-erasing .pip-scrub`, `.pip-catcher[data-pose] .pip-catch-pose`, `.tutorial-help-pip .tutorial-help-mark`) have to stay inline.
...
Every idle loop is infinite on purpose: the screenshot harness waits for finite animations to finish, so a loop that never settles must never claim to. One-shot moves are finite: the splat, the cheer, the staple, the eraser sweep, the catch, the boredom yawn, the theme flinch, and the hub wiggle. Everything switches off under `prefers-reduced-motion: reduce`.
...
- It contains the header with the window the data covers and the generation date, a dashboard per screen (see below), every insight card (the screen shows only the first six), the closing "Pip says" tip, and the all-time stats. Exporting from the Insights screen uses the range selected there; exporting from anywhere else uses the default last-12-months range. The header names the months rather than the range, `Feb 2024 to Jan 2025` and not `Last 12 months`: the window is anchored on the newest thing in the file rather than on today, which reads correctly on a screen and reads as a lie in a document that outlives the day it was made.
