...
Three layers guard parity. The hand-written ``*-parity.csv`` fixtures pin exact
...
(``parity-corpus-expected.json``, produced by the web cleaner). The shared
malformed-CSV manifest pins the normalized ``accepted``, ``empty_input``,
``invalid_header``, and ``parse_error`` outcomes while leaving each runtime's
user-facing error text unchanged. A cleaning change in only one runtime then
fails ``make test``.
"""
...
MALFORMED_OUTCOME_MANIFEST = json.loads(
    (FIXTURES_DIR / "malformed-csv-outcomes.json").read_text(encoding="utf-8"),
)
MALFORMED_OUTCOME_CATEGORIES = frozenset(
...
def _normalize_python_csv_outcome(result: CleanerResult) -> str:
    """Map Python cleaner results to the manifest's stable test vocabulary."""
    if result.success:
        return "accepted"
    ...
    if "No columns to parse from file" in error:
        return "empty_input"
    if error.startswith(
        ("Duplicate columns after header normalization:", "Missing required columns:")
    ):
...
def test_malformed_csv_outcome_manifest_contract() -> None:
    """The shared manifest should use only the documented stable vocabulary."""
    assert set(MALFORMED_OUTCOME_MANIFEST["categories"]) == MALFORMED_OUTCOME_CATEGORIES
    assert {case["boundary"] for case in MALFORMED_OUTCOME_CASES} == {"valid", "malformed"}
    assert len({case["id"] for case in MALFORMED_OUTCOME_CASES}) == len(MALFORMED_OUTCOME_CASES)
    for case in MALFORMED_OUTCOME_CASES:
        assert set(case["expected"]) == {"python", "web"}
        assert set(case["expected"].values()) <= MALFORMED_OUTCOME_CATEGORIES


@pytest.mark.parametrize(
    "case",
    MALFORMED_OUTCOME_CASES,
    ids=[case["id"] for case in MALFORMED_OUTCOME_CASES],
)
def test_malformed_csv_outcome_matches_python(case: dict[str, object], tmp_path: Path) -> None:
    """Python outcomes should match the shared malformed-CSV manifest."""
    file_type = str(case["fileType"])
    csv_text = case["csv"]
    ...
    result = CORPUS_CLEANERS[file_type](input_path=input_file, output_path=output_file)

    assert _normalize_python_csv_outcome(result) == expected["python"]
