...
def test_find_replacement_targets_covers_table_alignment_helper() -> None:
    """Direct table alignment maps to its supported Make target."""
    targets = check_doc_commands.find_replacement_targets(
        "python scripts/lint/align_tables.py README.md",
        {"align-tables"},
    )

    assert targets == ["align-tables"]
