...
describe('AnalyticsPage', () => {
    ...
    function sendViewResponse(requestId, viewOverrides = {}) {
        const view = {
            ...
            peakHour: { hour: 9 },
            streaks: { current: 2 },
            timeline: Array.from({ length: 10 }, (_, idx) => ({ label: String(idx), value: idx + 1 })),
            timelineMax: 10,
    ...
    it('animates timeline when conditions allow', async () => {
        ...
        await new Promise(resolve => setTimeout(resolve, 0));
        const viewRequest = workerInstance.postMessage.mock.calls.find(call => call[0].type === 'view');
    ...
    it('sets empty state when worker sends error message', async () => {
        ...
        expect(document.getElementById('analyticsEmpty').hidden).toBe(false);
        expect(document.getElementById('analyticsEmpty').querySelector('p').textContent)
            .toContain('Something went wrong in worker');
    });
    ...
    it('sets empty state when worker fires onerror event', async () => {
        ...
        expect(document.getElementById('analyticsEmpty').hidden).toBe(false);
        expect(document.getElementById('analyticsEmpty').querySelector('h2').textContent)
    ...
    it('defers render via resize retry when canvases are zero-sized and ResizeObserver absent', async () => {
        ...
        zeroTimeline.getBoundingClientRect = () => ({ width: 0, height: 0, left: 0, top: 0, right: 0, bottom: 0 });
        ...
        zeroTopics.getBoundingClientRect = () => ({ width: 0, height: 0, left: 0, top: 0, right: 0, bottom: 0 });
        ...
        zeroHeatmap.getBoundingClientRect = () => ({ width: 0, height: 0, left: 0, top: 0, right: 0, bottom: 0 });
        ...
        if (viewCall) {
            const view = {
                ...
                peakHour: { hour: 0 },
                streaks: { current: 1 },
                timeline: [{ label: 'Jan', value: 1 }, { label: 'Feb', value: 2 }],
                timelineMax: 2,
                topics: [],
                heatmap: {}
            };
    ...
    it('serializes non-default filter values into route on time range change', async () => {
        const requestId = await bootstrapWithData({ range: '3m', topic: 'Engineering', month: '2024-06', day: '4', hour: '14' });
    ...
    it('tooltip overflow clamps to viewport edges (lines 789, 792)', async () => {
        ...
        tooltip.getBoundingClientRect = () => ({
            width: 200, height: 100, left: 0, top: 0, right: 200, bottom: 100
        });
    ...
    it('scheduleViewRequest uses debounce delay when force=false', async () => {
        ...
        const callCountBefore = workerInstance.postMessage.mock.calls.filter(c => c[0].type === 'view').length;
        ...
        const countMid = workerInstance.postMessage.mock.calls.filter(c => c[0].type === 'view').length;
    ...
    it('week chart click sets monthFocus to item.monthKey (lines 749-751)', async () => {
        ...
        SketchCharts.getItemAt.mockReturnValue({ type: 'week', key: 'week-4', monthKey: '2024-03' });
        const canvas = document.getElementById('timelineChart');
        AppRouter.setParams.mockClear();
        canvas.dispatchEvent(new MouseEvent('click', { clientX: 10, clientY: 10 }));
