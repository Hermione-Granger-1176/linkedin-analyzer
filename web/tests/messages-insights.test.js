...
function buildDom(opts = {}) {
    const rangeButtons = opts.rangeButtons ?? `
...
function makeMessageState(overrides = {}) {
    ...
    return {
        contacts: new Map([
            ['c1', { name: 'Ada Lovelace', url: 'https://linkedin.com/in/ada', lastTimestamp: timestamp }]
        ]),
...
describe('MessagesPage', () => {
    beforeEach(async () => {
        ...
        MessagesAnalytics.normalizeName.mockImplementation(value => String(value || '').trim().toLowerCase());
    ...
    it('renders cached state and exports top contacts', async () => {
        ...
        expect(document.getElementById('messagesTipText').textContent).toContain('Upload Connections.csv');
    ...
    it('shows connection error message in lists and tip', async () => {
        ...
        await tick();
        expect(document.getElementById('silentConnectionsList').innerHTML)
            .toContain('Unable to parse Connections.csv.');
        expect(document.getElementById('messagesTipText').textContent)
            .toContain('Unable to parse Connections.csv.');
    });
    ...
    it('renders silent connections list when connections file is present', async () => {
        const messagesFile = { type: 'messages', name: 'm.csv', text: 'x', updatedAt: 50, rowCount: 1 };
        DataCache.set('storage:file:messages', messagesFile);

        const timestamp = new Date('2024-01-15').getTime();
        const messageState = {
            contacts: new Map([['c1', { name: 'Ada Lovelace', url: 'https://li.com/ada', lastTimestamp: timestamp }]]),
            events: [{ contactKey: 'c1', timestamp }],
            rowTimestamps: [timestamp],
            skippedRows: 0,
            talkedNameKeys: new Set(['ada lovelace']),
    ...
    it('renders silent connection without role info', async () => {
        const messagesFile = { type: 'messages', name: 'm2.csv', text: 'x', updatedAt: 51, rowCount: 1 };
        DataCache.set('storage:file:messages', messagesFile);
    ...
    it('renders fading conversations for connected contacts inactive > 30 days', async () => {
        ...
        const messagesFile = { type: 'messages', name: 'fading.csv', text: 'x', updatedAt: 60, rowCount: 1 };
        DataCache.set('storage:file:messages', messagesFile);
        ...
        const messageState = {
            contacts: new Map([
                ['c1', { name: 'Old Friend', url: 'https://li.com/old', lastTimestamp: oldTimestamp }]
            ]),
    ...
    it('shows empty fading list when no contacts are inactive > 30 days', async () => {
        ...
        const messagesFile = { type: 'messages', name: 'recent.csv', text: 'x', updatedAt: 61, rowCount: 1 };
        DataCache.set('storage:file:messages', messagesFile);
        ...
        const messageState = {
            contacts: new Map([
                ['c1', { name: 'Active Person', url: 'https://li.com/active', lastTimestamp: recentTimestamp }]
            ]),
    ...
    it('exports silent connections when data is available', async () => {
        ...
        const messagesFile = { type: 'messages', name: 'sc.csv', text: 'x', updatedAt: 70, rowCount: 1 };
        DataCache.set('storage:file:messages', messagesFile);
    ...
    it('exports fading conversations when data is available', async () => {
        ...
        const messagesFile = { type: 'messages', name: 'fc.csv', text: 'x', updatedAt: 80, rowCount: 1 };
        DataCache.set('storage:file:messages', messagesFile);
    ...
    it('time-range button click updates active state and re-renders', async () => {
        ...
        const timestamp = new Date('2024-06-01').getTime();
        const messagesFile = { type: 'messages', name: 'rng.csv', text: 'x', updatedAt: 90, rowCount: 1 };
        DataCache.set('storage:file:messages', messagesFile);
    ...
    it('reset filters button restores 12m range', async () => {
        ...
        const timestamp = new Date('2024-06-01').getTime();
        const messagesFile = { type: 'messages', name: 'rst.csv', text: 'x', updatedAt: 95, rowCount: 1 };
        DataCache.set('storage:file:messages', messagesFile);
        ...
        expect(document.querySelector('[data-range="12m"]').classList.contains('active')).toBe(true);
        expect(document.querySelector('[data-range="3m"]').classList.contains('active')).toBe(false);
    });
    ...
    it('hydrateMessageState reconstructs Maps and Sets from worker payload', async () => {
        ...
        const mockWorker = {
            ...
            addEventListener: vi.fn((type, handler) => {
                if (type === 'message') {workerMsgHandler = handler;}
            }),
            removeEventListener: vi.fn(),
            terminate: vi.fn()
        };
        globalThis.Worker = function () { return mockWorker; };
        ...
        const messagesFile = { type: 'messages', name: 'w.csv', text: 'csv', updatedAt: 100, rowCount: 5 };
        DataCache.set('storage:file:messages', messagesFile);
    ...
    it('falls back to main-thread when worker fires error event', async () => {
        ...
        const mockWorker = {
            postMessage: vi.fn(),
            addEventListener: vi.fn((type, handler) => {
                if (type === 'error') {workerErrHandler = handler;}
            }),
            removeEventListener: vi.fn(),
            terminate: vi.fn()
        };
        ...
        const timestamp = new Date('2024-03-01').getTime();
        const messagesFile = { type: 'messages', name: 'werr.csv', text: 'csv', updatedAt: 110, rowCount: 2 };
        DataCache.set('storage:file:messages', messagesFile);
        ...
        MessagesAnalytics.buildConnectionState.mockReturnValue({ list: [], byUrl: new Map(), byName: new Map() });
    ...
    it('falls back to main-thread when worker times out', async () => {
        ...
        const timestamp = new Date('2024-03-01').getTime();
        const messagesFile = { type: 'messages', name: 'timeout.csv', text: 'csv', updatedAt: 120, rowCount: 2 };
        DataCache.set('storage:file:messages', messagesFile);
        ...
        const messageState = {
            contacts: new Map([['c1', { name: 'Timeout User', url: '', lastTimestamp: timestamp }]]),
            events: [{ contactKey: 'c1', timestamp }],
            rowTimestamps: [timestamp],
            skippedRows: 0,
    ...
    it('parses on main thread when Worker is unavailable', async () => {
        ...
        const timestamp = new Date('2024-03-01').getTime();
        const messagesFile = { type: 'messages', name: 'noworker.csv', text: 'csv', updatedAt: 130, rowCount: 1 };
        DataCache.set('storage:file:messages', messagesFile);
    ...
    it('shows parse error empty state when main-thread parse fails', async () => {
        ...
        const messagesFile = { type: 'messages', name: 'bad.csv', text: 'bad', updatedAt: 140, rowCount: 0 };
        DataCache.set('storage:file:messages', messagesFile);
        ...
        expect(document.getElementById('messagesEmpty').hidden).toBe(false);
        expect(document.getElementById('messagesEmpty').querySelector('h2').textContent)
            .toContain('Messages parsing error');
    ...
    it('shows empty state when parsed data has no valid rows', async () => {
        ...
        const messagesFile = { type: 'messages', name: 'empty.csv', text: 'x', updatedAt: 141, rowCount: 0 };
        DataCache.set('storage:file:messages', messagesFile);
        ...
        expect(document.getElementById('messagesEmpty').hidden).toBe(false);
        expect(document.getElementById('messagesEmpty').querySelector('p').textContent)
            .toContain('no valid message rows');
    ...
    it('updateStats shows plain number when skippedRows is zero', async () => {
        const messagesFile = { type: 'messages', name: 'plain.csv', text: 'x', updatedAt: 150, rowCount: 1 };
        DataCache.set('storage:file:messages', messagesFile);
    ...
    it('skipped-row popup toggles on click and closes on Escape', async () => {
        const messagesFile = { type: 'messages', name: 'skip.csv', text: 'x', updatedAt: 160, rowCount: 5 };
        DataCache.set('storage:file:messages', messagesFile);
    ...
    it('skipped-row popup opens on Enter key', async () => {
        const messagesFile = { type: 'messages', name: 'skipenter.csv', text: 'x', updatedAt: 161, rowCount: 3 };
        DataCache.set('storage:file:messages', messagesFile);
        const messageState = makeMessageState({ skippedRows: 1 });
        const sig = `messages:${messagesFile.name}:${messagesFile.updatedAt}:${messagesFile.rowCount}|connections:none`;
        DataCache.set(`messages:state:${sig}`, {
            messageState, connectionState: null, connectionLoadError: null, hasConnectionsFile: false
        });
    ...
    it('skipped-row popup closes on focusout', async () => {
        const messagesFile = { type: 'messages', name: 'skipfocus.csv', text: 'x', updatedAt: 162, rowCount: 3 };
        DataCache.set('storage:file:messages', messagesFile);
        const messageState = makeMessageState({ skippedRows: 1 });
        const sig = `messages:${messagesFile.name}:${messagesFile.updatedAt}:${messagesFile.rowCount}|connections:none`;
        DataCache.set(`messages:state:${sig}`, {
    ...
    it('skipped-row popup closes on mouseleave', async () => {
        const messagesFile = { type: 'messages', name: 'skipmouse.csv', text: 'x', updatedAt: 163, rowCount: 3 };
        DataCache.set('storage:file:messages', messagesFile);
        const messageState = makeMessageState({ skippedRows: 1 });
        const sig = `messages:${messagesFile.name}:${messagesFile.updatedAt}:${messagesFile.rowCount}|connections:none`;
        DataCache.set(`messages:state:${sig}`, {
    ...
    it('tip shows both top contact and fading contact when both exist', async () => {
        ...
        const messagesFile = { type: 'messages', name: 'tip2.csv', text: 'x', updatedAt: 170, rowCount: 2 };
        DataCache.set('storage:file:messages', messagesFile);
    ...
    it('tip shows only top contact message when no fading contacts', async () => {
        ...
        const messagesFile = { type: 'messages', name: 'tip3.csv', text: 'x', updatedAt: 171, rowCount: 1 };
        DataCache.set('storage:file:messages', messagesFile);
    ...
    it('tip shows silent-connections count when no top contact but silent list non-empty', async () => {
        const messagesFile = { type: 'messages', name: 'tip4.csv', text: 'x', updatedAt: 172, rowCount: 0 };
        DataCache.set('storage:file:messages', messagesFile);
    ...
    it('reuses cached state when data signature is unchanged on second route change', async () => {
        const messagesFile = { type: 'messages', name: 'reuse.csv', text: 'x', updatedAt: 180, rowCount: 1 };
        DataCache.set('storage:file:messages', messagesFile);
    ...
    it('matches fading contact to connection by name key when URL missing', async () => {
        ...
        const messagesFile = { type: 'messages', name: 'namekey.csv', text: 'x', updatedAt: 190, rowCount: 1 };
        DataCache.set('storage:file:messages', messagesFile);
    ...
    it('loads files from storage when not in cache', async () => {
        ...
        Storage.getAllFiles.mockResolvedValue([
            { type: 'messages', name: 'fromdb.csv', text: 'csv-content', updatedAt: 200, rowCount: 1 },
            { type: 'connections', name: 'Connections.csv', text: 'conn-content', updatedAt: 200, rowCount: 2 }
        ]);
    ...
    it('falls back to main-thread when worker postMessage throws', async () => {
        ...
        globalThis.Worker = function () { return throwingWorker; };
        ...
        const timestamp = new Date('2024-03-01').getTime();
        const messagesFile = { type: 'messages', name: 'throw.csv', text: 'csv', updatedAt: 210, rowCount: 1 };
        DataCache.set('storage:file:messages', messagesFile);
    ...
    it('records connection parse error when connections CSV fails on main thread', async () => {
        ...
        Storage.getAllFiles.mockResolvedValue([
            ...
            { type: 'connections', name: 'Connections.csv', text: 'bad-conn', updatedAt: 220, rowCount: 0 }
        ]);
        ...
        expect(document.getElementById('silentConnectionsList').innerHTML).toContain('Bad conn CSV');
    ...
    it('filters top contacts to the selected time range', async () => {
        ...
        const messagesFile = { type: 'messages', name: 'range.csv', text: 'x', updatedAt: 230, rowCount: 2 };
        DataCache.set('storage:file:messages', messagesFile);
    ...
    it('renders contact name as link when URL is present', async () => {
        const timestamp = new Date('2024-06-01').getTime();
        const messagesFile = { type: 'messages', name: 'link.csv', text: 'x', updatedAt: 240, rowCount: 1 };
        DataCache.set('storage:file:messages', messagesFile);

        const messageState = {
            contacts: new Map([
                ['c1', { name: 'Link Person', url: 'https://linkedin.com/in/link', lastTimestamp: timestamp }]
            ]),
            events: [{ contactKey: 'c1', timestamp }],
            rowTimestamps: [timestamp],
            skippedRows: 0,
        ...
        expect(document.getElementById('topContactsList').innerHTML)
            .toContain('<a href="https://linkedin.com/in/link"');
    });
    ...
    it('hydrateConnectionState populates byName map from connection list', async () => {
        ...
        const messagesFile = { type: 'messages', name: 'hyd.csv', text: 'csv', updatedAt: 250, rowCount: 1 };
        const connectionsFile = { type: 'connections', name: 'Connections.csv', text: 'csv', updatedAt: 250, rowCount: 1 };
        DataCache.set('storage:file:messages', messagesFile);
            ...
            workerMsgHandler({
                data: {
                    ...
                    payload: {
                        ...
                        totalInputRows: 1,
                        messageState: {
                            contacts: [{ key: 'c1', name: 'Hyd User', url: '', lastTimestamp: oldTs }],
                            events: [{ contactKey: 'c1', timestamp: oldTs }],
                            rowTimestamps: [oldTs],
                            skippedRows: 0,
    ...
    it('export buttons are disabled when lists are empty', async () => {
        const messagesFile = { type: 'messages', name: 'noexp.csv', text: 'x', updatedAt: 260, rowCount: 0 };
        DataCache.set('storage:file:messages', messagesFile);
    ...
    it('ignores worker messages with mismatched requestId', async () => {
        ...
        const messagesFile = { type: 'messages', name: 'wrongid.csv', text: 'csv', updatedAt: 270, rowCount: 1 };
        DataCache.set('storage:file:messages', messagesFile);
    ...
    it('accumulates message count for contacts with multiple events', async () => {
        ...
        const messagesFile = { type: 'messages', name: 'multi.csv', text: 'x', updatedAt: 280, rowCount: 2 };
        DataCache.set('storage:file:messages', messagesFile);
    ...
    it('sorts silent connections by connectedOnTimestamp', async () => {
        ...
        const messagesFile = { type: 'messages', name: 'silentsort.csv', text: 'x', updatedAt: 290, rowCount: 1 };
        DataCache.set('storage:file:messages', messagesFile);

        const conn1 = {
            name: 'Zeta Connect', url: 'https://li.com/zeta', nameKey: 'zeta connect',
            connectedOnTimestamp: ts2  // newer
        };
        const conn2 = {
            name: 'Alpha Connect', url: 'https://li.com/alpha', nameKey: 'alpha connect',
        ...
        const connectionState = {
            list: [conn1, conn2],
            byUrl: new Map([['https://li.com/zeta', conn1], ['https://li.com/alpha', conn2]]),
            byName: new Map([['zeta connect', conn1], ['alpha connect', conn2]])
        };

        const sig = `messages:${messagesFile.name}:${messagesFile.updatedAt}:${messagesFile.rowCount}|connections:none`;
        DataCache.set(`messages:state:${sig}`, {
            messageState, connectionState, connectionLoadError: null, hasConnectionsFile: true
        });
    ...
    it('sorts fading conversations by lastTimestamp descending', async () => {
        ...
        const messagesFile = { type: 'messages', name: 'fadingsort.csv', text: 'x', updatedAt: 300, rowCount: 2 };
        DataCache.set('storage:file:messages', messagesFile);

        const conn1 = {
            name: 'Old Fader', url: 'https://li.com/old', nameKey: 'old fader',
            connectedOnTimestamp: t1, company: 'A'
        };
        const conn2 = {
            name: 'New Fader', url: 'https://li.com/new', nameKey: 'new fader',
            connectedOnTimestamp: t2, company: 'B'
        };
        const messageState = {
            contacts: new Map([
        ...
        const connectionState = {
            list: [conn1, conn2],
            byUrl: new Map([['https://li.com/old', conn1], ['https://li.com/new', conn2]]),
            byName: new Map([['old fader', conn1], ['new fader', conn2]])
        };
    ...
    it('getRangeStart returns null for "all" range (shows all contacts)', async () => {
        ...
        const messagesFile = { type: 'messages', name: 'allrange.csv', text: 'x', updatedAt: 310, rowCount: 1 };
        DataCache.set('storage:file:messages', messagesFile);

        const messageState = {
            contacts: new Map([
                ['c1', { name: 'Old Timer', url: '', lastTimestamp: old }]
            ]),
    ...
    // --- hydrateMessageState null contact guard (line 532) -------------------
    ...
    it('hydrateMessageState skips contacts with no key', async () => {
        ...
        const messagesFile = { type: 'messages', name: 'nullkey.csv', text: 'csv', updatedAt: 320, rowCount: 1 };
        DataCache.set('storage:file:messages', messagesFile);
