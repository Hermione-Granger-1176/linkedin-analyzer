...
describe("Storage fallback when IndexedDB is unavailable", () => {
    ...
    it("reports isAvailable as false", async () => {
        const { Storage } = await import("../../../src/platform/persistence/storage.js");
    ...
    it("normalizes missing rowCount to 0 and adds schemaVersion in memory mode", async () => {
        const { Storage } = await import("../../../src/platform/persistence/storage.js");
        await Storage.saveFile("shares", { name: "test.csv", text: "a,b" });
        const file = await Storage.getFile("shares");
    ...
    it("returns raw analytics payload shape in memory mode", async () => {
        const { Storage } = await import("../../../src/platform/persistence/storage.js");
        await Storage.saveAnalytics({ total: 7, nested: { ok: true } });
        expect(await Storage.getAnalytics()).toEqual({ total: 7, nested: { ok: true } });
    });
});
