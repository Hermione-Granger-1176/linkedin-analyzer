...
describe('UploadPage', () => {
    ...
    function setupUploadDom() {
        ...
        window.cancelAnimationFrame = vi.fn((id) => { /* no-op for sync rAF */ });
        ...
        window.requestIdleCallback = vi.fn((cb) => { cb(); return 0; });
    ...
    it('warns when storage is low during upload', async () => {
        ...
        await new Promise(resolve => setTimeout(resolve, 0));
        expect(document.getElementById('uploadHint').textContent).toContain('Storage is running low');
    ...
    it('shows error hint and resets processing state on worker error event', async () => {
        ...
        expect(document.getElementById('uploadHint').textContent).toContain('Analytics worker error');
        expect(document.getElementById('uploadHint').classList.contains('is-error')).toBe(true);
    ...
    it('handles error message type from worker with a jobId (completes job)', async () => {
        ...
        globalThis.FileReader = function FileReader() {
            return {
                ...
                onload: null,
                onerror: null,
                readAsText() { if (this.onload) { this.onload(); } }
            };
        };
        ...
        await workerInstance.listeners.message[0]({
            data: {
                type: 'error',
                payload: { message: 'Job failed', jobId: 'some-job-id', fileName: 'Connections.csv' }
            }
        });
    ...
    it('shows error hint when fileProcessed payload has no fileType', async () => {
        ...
        expect(document.getElementById('uploadHint').textContent).toContain('Unrecognized file type');
    ...
    it('hides progress overlay after all jobs complete', async () => {
        ...
        globalThis.FileReader = function FileReader() { return fileReaderInstance; };
    ...
    it('schedules prime via setTimeout when requestIdleCallback is unavailable', async () => {
        ...
        let rafCounter = 0;
        window.requestAnimationFrame = vi.fn((cb) => { cb(rafCounter++ * 700); return rafCounter; });
    ...
    it('does not re-prime worker when signature is unchanged', async () => {
        ...
        const firstRestoreCount = workerInstance.postMessage.mock.calls
            .filter(c => c[0] && c[0].type === 'restoreFiles').length;
        ...
        const secondRestoreCount = workerInstance.postMessage.mock.calls
            .filter(c => c[0] && c[0].type === 'restoreFiles').length;
    ...
    it('completes job and shows timeout hint when job times out', async () => {
        ...
        expect(document.getElementById('uploadHint').textContent).toContain('Processing took too long');
        expect(document.getElementById('uploadHint').classList.contains('is-error')).toBe(true);
    ...
    it('restores file status from storage on load and enables analytics button', async () => {
        ...
        expect(document.querySelector('.file-status-item[data-file="shares"]').classList.contains('is-ready'))
            .toBe(true);
        expect(document.querySelector('.file-status-item[data-file="comments"]').classList.contains('is-ready'))
            .toBe(true);
        expect(document.getElementById('openAnalyticsBtn').disabled).toBe(false);
    });
    ...
    it('onRouteChange syncs from cache when already initialized and restored', async () => {
        ...
        expect(document.querySelector('.file-status-item[data-file="messages"]').classList.contains('is-ready'))
            .toBe(true);
    });
    ...
    it('processes multiple CSV files dropped at once', async () => {
        ...
        const addFileCalls = workerInstance.postMessage.mock.calls
            .filter(c => c[0] && c[0].type === 'addFile');
    ...
    it('shows error hint and completes job when FileReader errors', async () => {
        ...
        globalThis.FileReader = function FileReader() {
            return {
                ...
                onload: null,
                onerror: null,
                readAsText() { if (this.onerror) { this.onerror(new Error('read error')); } }
            };
        };
    ...
    it('shows hint when worker is unavailable during file upload', async () => {
        ...
        expect(document.getElementById('uploadHint').textContent).toContain('Workers are unavailable');
    ...
    it('shows "Files loaded. Open Messages tab" hint when only messages file present', async () => {
        ...
        expect(document.getElementById('uploadHint').textContent)
            .toContain('Files loaded. Open Messages tab');
    });
    ...
    it('shows "Upload at least one file" hint when no files are present', async () => {
        ...
        expect(document.getElementById('uploadHint').textContent)
            .toContain('Upload at least one file');
    });
    ...
    it('waits for session cleanup promise on window before restoring', async () => {
        let resolveCleanup;
        window.__linkedinAnalyzerSessionCleanupPromise = new Promise(
            resolve => { resolveCleanup = resolve; }
        );
    ...
    it('drawProgressBar does not throw when canvas has zero dimensions', async () => {
        ...
        canvas.id = 'progressCanvas';
        canvas.getBoundingClientRect = () => ({ width: 0, height: 0, left: 0, top: 0, right: 0, bottom: 0 });
        const existingCanvas = document.getElementById('progressCanvas');
        existingCanvas.replaceWith(canvas);
    ...
    it('startProgressCrawl advances progress while a job is active', async () => {
        ...
        globalThis.FileReader = function FileReader() {
            return {
                ...
                readAsText() {
                    this.result = 'col\nval';
                    triggerLoad = () => { if (this.onload) { this.onload(); } };
                }
            };
        };
    ...
    it('shows server hint when Worker constructor throws and a file is uploaded', async () => {
        ...
        globalThis.Worker = function () { throw new Error('no workers'); };
    ...
    it('does not show low storage hint when estimate returns null', async () => {
        ...
        expect(document.getElementById('uploadHint').textContent)
            .not.toContain('Storage is running low');
    ...
    it('clears active job timeouts when worker error occurs during processing', async () => {
        ...
        expect(document.getElementById('uploadHint').textContent)
            .toContain('Analytics worker error');
    ...
    it('uses cached files snapshot when available in DataCache', async () => {
        const cachedFiles = [
            { type: 'shares', name: 'Shares.csv', rowCount: 3, updatedAt: 500 }
        ];
        DataCache.get.mockImplementation((key) => {
            if (key === 'storage:files') {return cachedFiles;}
            if (key === 'analytics:ready') {return true;}
            return null;
        });
    ...
    it('primes worker via immediate path when no prior signature exists', async () => {
        ...
        Storage.getAllFiles.mockResolvedValue([
            { type: 'shares', name: 'Shares.csv', rowCount: 2, text: 'col\nval\nval2', updatedAt: 600 }
        ]);
        ...
        const restoreCall = workerInstance.postMessage.mock.calls
            .find(c => c[0] && c[0].type === 'restoreFiles');
        expect(restoreCall).toBeTruthy();
    ...
    it('animateProgressTo stops when session ID changes mid-animation', async () => {
        ...
        globalThis.FileReader = function FileReader() {
            const inst = {
                ...
                readAsText() {
                    fileCount++;
                    if (fileCount === 1) {
                        triggerLoad1 = () => { if (inst.onload) { inst.onload(); } };
                    } else {
                        triggerLoad2 = () => { if (inst.onload) { inst.onload(); } };
                    }
                }
            };
            return inst;
        ...
        if (triggerLoad1) { triggerLoad1(); }
        if (triggerLoad2) { triggerLoad2(); }
    ...
    it('syncStatusFromCache shows analytics-ready state from full cache', async () => {
        ...
        const sharesFile = { type: 'shares', name: 'Shares.csv', rowCount: 5, text: 'col\nv', updatedAt: 800 };
        DataCache.get.mockImplementation((key) => {
            if (key === 'storage:files') {return [sharesFile];}
            if (key === 'storage:analyticsBase') {
    ...
    it('getStoredFilesSnapshot uses DataCache when files are cached', async () => {
        ...
        const cachedFile = { type: 'messages', name: 'Messages.csv', rowCount: 1, text: 'col\nval', updatedAt: 900 };
        DataCache.get.mockImplementation((key) => {
            if (key === 'storage:files') {return [cachedFile];}
            return null;
        });
    ...
    it('clicking the drop zone triggers file input click', async () => {
        ...
        document.getElementById('multiDropZone').dispatchEvent(new MouseEvent('click', { bubbles: true }));
    ...
    it('syncTypeSpecificFileCache returns early when file type not in stored files', async () => {
        ...
        const typeKeyCall = DataCache.set.mock.calls
            .find(c => c[0] === 'storage:file:messages');
    ...
    it('consumePendingFile resolves by jobId when worker echoes the jobId', async () => {
        ...
        const addFileCall = workerInstance.postMessage.mock.calls
            .find(c => c[0] && c[0].type === 'addFile');
    ...
    it('resolveJobId falls back to resolvePendingJobIdByFileName when jobId is null', async () => {
        ...
        globalThis.FileReader = function FileReader() {
            const inst = {
                ...
                onerror: null,
                readAsText() {
                    triggerLoad = () => { if (inst.onload) {inst.onload();} };
                }
            };
            return inst;
        };
        ...
        Storage.getAllFiles.mockResolvedValue([
            { type: 'connections', name: 'Connections.csv', rowCount: 5, text: 'col\nval', updatedAt: 1 }
        ]);
    ...
    it('primeAnalyticsWorkerNow returns early when worker is null', async () => {
        ...
        const savedWorker = globalThis.Worker;
        globalThis.Worker = function() { throw new Error('no worker'); };
    ...
    it('clearPrimeSchedule cancels a pending setTimeout prime', async () => {
        ...
        window.requestAnimationFrame = vi.fn((cb) => { rafVirtual += 700; cb(rafVirtual); return 1; });
        window.cancelAnimationFrame = vi.fn();
    ...
    it('hideProgressOverlay callback returns early when session changes', async () => {
        ...
        globalThis.FileReader = function FileReader() {
            const inst = {
                ...
                readAsText() {
                    const idx = readCount++;
                    resolvers[idx] = () => { if (inst.onload) {inst.onload();} };
                }
            };
            return inst;
        };
